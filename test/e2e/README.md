...
- **Parallelism:** suites currently share one sandbox (one emulated `$HOME`, one workspace, one node/httpd on a fixed port) and the "uninstalled" emulation mutates it, so only one spec per session is safe and `maxInstances` is `1`. Specs are written to be self-contained and order-independent so a future move to parallel is isolated to the harness: give each worker its own emulated `$HOME` (copied from a template built once), its own workspace (via `wdio:vscodeOptions.workspacePath`), and its own node + httpd on a per-worker port (with `radicle.advanced.httpApiEndpoint` set per worker), then dispose them in `afterSession`. The node stays, since future suites (clone, sync, patches over httpd) need it live.
