...
import { TreeItem, ViewContent, ViewSection, Workbench } from 'wdio-vscode-service'
...
describe('VS Code Extension Testing', () => {
  ...
  it('should load and install our VSCode Extension', async () => {
    ...
    expect(
      extensions.some((extension: { id: string }) => extension.id === 'radicle-ide-plugins-team.radicle'),
    ).toBe(true)
  })
  ...
  describe.only('sections', () => {
    ...
    it('should show the correct text and buttons on the CLI Commands section', async () => {
      ...
      const buttonTitles = await Promise.all(buttons.map(button => button.getTitle()))
  
      expect(welcomeText.some(text => /Use the buttons below to perform common interactions with the Radicle network./i.test(text))).toBe(true)
    ...
    it('should be able to open a patch\'s details', async () => {
      ...
      await expect($('h1')).toHaveText('feat(config): implement runtime configuration via json file')
    })
  })
