...
import { exec, getConfigPathToRadBinary, isGitInitialised, log, setConfigPathToRadBinary, setWhenClauseContext } from '.';
...
export async function getRadCliStatusMsg(): Promise<string> {
  ...
  if (await isRadCliInstalled()) {
    ...
    msg = `Failed resolving Radicle CLI binary. Tried invoking it in the shell as "${
      getRadCliRef()
    }".`;
  }
...
export async function troubleshootRadCliInstallation(): Promise<void> {
  ...
  if (pathSetMethodSelection === filePicker) {
    const fileUriSelection = (await window.showOpenDialog({
      title,
      openLabel: 'Select',
      canSelectMany: false,
    }))?.[0];
  ...
  if (pathSetMethodSelection === inputBox) {
    const typedInPath = (await window.showInputBox({
      title,
